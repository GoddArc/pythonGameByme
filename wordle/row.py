...
class Row(pygame.sprite.Sprite):
    def __init__(self,y):
        super().__init__()
        pygame.init()
        ...
        if len(Word)%2==0:
            self.rect.x=Width//2-(gap/2+(int(len(Word)/2)*square_Size)+((len(Word)/2)-1)*gap)
        else:
            self.rect.x=Width//2-((square_Size//2)+(int(len(Word)/2)*(square_Size+gap)))
        self.rect.y=y
        self.sq_group=[]
        self.pos_letter=[]
        self.color=['grey']*len(Word)
